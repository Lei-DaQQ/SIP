<template>
  <div>
    ...
    <div class="mapMain">
      <el-table :data="maps" border @selection-change="handleSelectionChange" size="small" stripe style="width: 50%">
        ...
        <el-table-column prop="keyword" width="250" label="坐标"></el-table-column>
<!--        <el-table-column prop="createDate" width="150" label="创建时间"></el-table-column>-->
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            <el-button size="mini" @click="showLookView(scope.$index, scope.row)" icon="fa fa-search"></el-button>
            <el-button size="mini" type="danger" @click="handleDelete(scope.$index,scope.row)" icon="fa fa-trash-o"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="地图展示" :visible.sync="dialogVisible_show" width="80%">
      <el-container>
        <el-aside style="width: 50%;">
          <vl-map :load-tiles-while-animating="true" :load-tiles-while-interacting="true" style="height: 600px;" data-projection="EPSG:4326">
    ...
    <el-dialog title="增加地图" :visible.sync="dialogVisible_add" width="40%">
      <el-form :model="addMap" :label-width="formLabelWidth">
        <el-form-item label="地图名称">
          <el-input class="addMapInput" size="small" v-model="AddMap.name" prefix-icon="el-icon-edit" placeholder="请输入地图名称"></el-input>
        </el-form-item>
        <el-form-item label="地图坐标">
          <el-input class="addMapInput" size="small" v-model="AddMap.keyword" prefix-icon="fa fa-map-pin" placeholder="请输入地图坐标"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    deleteMany() {
      this.$confirm(' 此操作将永久删除【' + this.multipleSelection.length + '】条记录, 是否继续?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}
      ).then(() => {
        let ids = '?';
